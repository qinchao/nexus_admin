    ...
    render: (text, item) => (
      <span>
        ...
        item.status === "WAITING_FOR_INVESTIGATION" ? (
          <Link
            to={`${routerConfig.operation.withdrawInspection}?userId=${item.userId}&recordId=${item.recordId}&currency=${item.currency}&inspect=true`}
          >
          ...
          <Link
            to={`${routerConfig.operation.withdrawInspection}?userId=${item.userId}&recordId=${item.recordId}&inspect=false`}
          >
            <Button className="resetButton" type="primary">
              Review
            </Button>
